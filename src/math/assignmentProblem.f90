...
contains
    ...
    subroutine initialize(n, A, f, ff, u, v)
        ...
        do i=1,n
            if (f(i) == 0) then
                ...
                tmp = huge(A)
                j = 1 ! initialize j, in case first iteration goes to tie breaking (can happen if 'huge' is in A)
                do k=1,n
    ...
    subroutine path(n, A, ff, u, v, c, i, j)
        ...
        do
            ...
            ! we go and create one by substracting d (the smallest possible number that can create one)
            if (nz) then
                do x=1,nlr
                    u(lr(x)) = u(lr(x)) + d
                end do
